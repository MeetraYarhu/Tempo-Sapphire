        ...
        if (args.length !== 3) { // send appropriate error if arguments are not sufficient length
            ...
            if (!speedSelection) {
                ...
                for (let i = 0; i < guildStuff.guilds.length; i++) {
                    this.client.channels.cache.get(guildStuff.guilds[i].channelid).send(`${worldSelection.name} ShB train starting at ${locationSelection.name}, ${locationSelection.zone}\n<@&${guildStuff.guilds[i].trainroleid}> ${worldSelection.roleid[i]}`, {
                        embed: relayEmbed,
                    });
